package nc.dssl;

import javax.annotation.Nullable;
import java.util.*;
import java.util.Map.Entry;
import java.util.function.BiConsumer;

public class DSSLHierarchy<K, V> {
	
	protected final Map<K, V> internal;
	protected final List<DSSLHierarchy<K, V>> parents;
	
	public DSSLHierarchy() {
		this(new HashMap<>(), Arrays.asList());
	}
	
	protected DSSLHierarchy(Map<K, V> internal, List<DSSLHierarchy<K, V>> parents) {
		this.internal = internal;
		this.parents = parents;
	}
	
	public boolean containsKey(K key, boolean shallow) {
		if (internal.containsKey(key)) {
			return true;
		}
		if (!shallow) {
			for (DSSLHierarchy<K, V> parent : parents) {
				if (parent.containsKey(key, false)) {
					return true;
				}
			}
		}
		return false;
	}
	
	public @Nullable V get(K key, boolean shallow) {
		if (internal.containsKey(key)) {
			return internal.get(key);
		}
		if (!shallow) {
			for (DSSLHierarchy<K, V> parent : parents) {
				V value = parent.get(key, false);
				if (value != null) {
					return value;
				}
			}
		}
		return null;
	}
	
	protected @Nullable V putInternal(K key, V value, boolean shadow) {
		if (shadow || internal.containsKey(key)) {
			return internal.put(key, value);
		}
		for (DSSLHierarchy<K, V> parent : parents) {
			V prev = parent.putInternal(key, value, false);
			if (prev != null) {
				return prev;
			}
		}
		return null;
	}
	
	public @Nullable V put(K key, V value, boolean shadow) {
		V prev = putInternal(key, value, shadow);
		if (!shadow && prev == null) {
			throw new IllegalArgumentException(String.format("Encountered unexpected key \"%s\"!", key));
		}
		return prev;
	}
	
	public @Nullable V remove(K key, boolean shallow) {
		if (internal.containsKey(key)) {
			return internal.remove(key);
		}
		if (!shallow) {
			for (DSSLHierarchy<K, V> parent : parents) {
				V value = parent.remove(key, false);
				if (value != null) {
					return value;
				}
			}
		}
		return null;
	}
	
	public void forEach(BiConsumer<K, V> consumer, boolean shallow) {
		for (Entry<K, V> entry : internal.entrySet()) {
			consumer.accept(entry.getKey(), entry.getValue());
		}
		if (!shallow) {
			for (DSSLHierarchy<K, V> parent : parents) {
				parent.forEach(consumer, false);
			}
		}
	}
	
	public DSSLHierarchy<K, V> copy(boolean child) {
		return child ? new DSSLHierarchy<>(new HashMap<>(), Arrays.asList(this)) : this;
	}
	
	public DSSLHierarchy<K, V> branch(List<DSSLHierarchy<K, V>> parents) {
		return new DSSLHierarchy<>(internal, parents);
	}
}
